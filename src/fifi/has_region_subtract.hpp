...
namespace fifi
{
    ...
    /// region_subtract(value_type*,value_type*,uint32_t)
    ///
    /// Example:
    ///
    /// typedef fifi::simple_online online;
    ///
    /// if(kodo::has_region_subtract<online>::value)
    /// {
    ///     // Do something here
    /// }
    ///
    template<typename T>
    struct has_region_subtract
    {
    private:

        template<typename U>
        static auto test(int) ->
            decltype(std::declval<U>().region_subtract(0,0,0), uint32_t());
