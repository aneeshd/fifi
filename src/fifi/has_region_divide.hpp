...
namespace fifi
{
    ...
    /// region_divide(value_type*,value_type*,uint32_t)
    ///
    /// Example:
    ///
    /// typedef fifi::simple_online online;
    ///
    /// if(kodo::has_region_divide<online>::value)
    /// {
    ///     // Do something here
    /// }
    ///
    template<typename T>
    struct has_region_divide
    {
    private:

        template<typename U>
        static auto test(int) ->
            decltype(std::declval<U>().region_divide(0,0,0), uint32_t());
