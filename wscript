...
def configure(conf):
    ...
    if not conf.bundle_dependency('gtest'):
        if not conf.options.gtest_path: conf.fatal('Either bundle gtest of specify its path')
        else: conf.recurse(conf.options.gtest_path)
    else:
        conf.recurse_dependency('gtest')

    if not conf.bundle_dependency('boost'):
        if not conf.options.boost_path: conf.fatal('Either bundle boost of specify its path')
        else: conf.recurse(conf.options.boost_path)
    else:
