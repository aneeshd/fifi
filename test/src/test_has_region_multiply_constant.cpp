...
#include <fifi/has_region_multiply_constant.hpp>
...
TEST(test_has_region_multiply_constant, api)
{
    ...
    EXPECT_TRUE(fifi::has_region_multiply_constant<
                    fifi::simple_online<fifi::binary>>::value);

    EXPECT_TRUE(fifi::has_region_multiply_constant<
                    fifi::full_table<fifi::binary8>>::value);

    EXPECT_FALSE(fifi::has_region_multiply_constant<uint32_t>::value);
}
